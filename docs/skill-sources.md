...
ec2-cli vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. ec2-cli
...
#   - consumer-identifying prose: `guildmaster` → `ec2-cli` (NOT
...
If a re-sync would lose a ec2-cli adaptation, lift the change
...
#     → `which ec2-cli vendors from guildmaster`
