...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope ec2-cli
...
scope:** the default personal scope is **private** (`--scope ec2-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `ec2-cli`). Private records are served only to a recall in the same
